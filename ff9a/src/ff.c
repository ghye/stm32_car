...
)
{
	...
	*rfs = fs;							/* Return pointer to the corresponding file system object */
	...
	if (fmt == 3) {
		debug_printf_m("check_fs err, fmt==3!!");
		return FR_DISK_ERR;
	}
	if (fmt) {
		debug_printf_s("fmt=");
	...
	debug_printf_m("check_fs ok");

	/* An FAT volume is found. Following code initializes the file system object */

	if (LD_WORD(fs->win+BPB_BytsPerSec) != SS(fs))	{	/* (BPB_BytsPerSec must be equal to the physical sector size) */
		debug_printf_m("check BPB_BytsPerSec err!!");
	...
	if (b != 1 && b != 2) {
		debug_printf_m("check BPB_NumFATs err!!");
	...
	if (!nrsv) {
		debug_printf_m("check BPB_RsvdSecCnt err!!");
	...
	if (tsect < sysect) {
		debug_printf_m("check RSV+FAT+DIR err!!");
		return FR_NO_FILESYSTEM;		/* (Invalid volume size) */
	}
	nclst = (tsect - sysect) / fs->csize;				/* Number of clusters */
	if (!nclst) {
		debug_printf_m("check nclst err!!");
		return FR_NO_FILESYSTEM;				/* (Invalid volume size) */
	}
	fmt = FS_FAT12;
	...
	if (fmt == FS_FAT32) {
		if (fs->n_rootdir) {
			debug_printf_m("check FS_FAT32 err!!");
		...
		if (!fs->n_rootdir)	{
			debug_printf_m("check n_rootdir err!!");
	...
	if (fs->fsize < (szbfat + (SS(fs) - 1)) / SS(fs))	{/* (BPB_FATSz must not be less than required) */
		debug_printf_m("check BPB_FATSz less err!!");
		return FR_NO_FILESYSTEM;
	}
